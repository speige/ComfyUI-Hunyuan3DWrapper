...
class HunyuanPaintPipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        **cached_condition,
        ...
        # device = getattr(self.unet, "device", prompt_embeds.device) # next(self.unet.parameters()).device
        prompt_embeds = prompt_embeds.to(device)
        negative_prompt_embeds = negative_prompt_embeds.to(device)
            
        # with torch.autocast(device_type="cuda", dtype=torch.float8_e4m3fn): # throws exception: "addmm_cuda"  not implemented for 'Float8_e4m3fn'
    ...
    def denoise(
        ...
        **kwargs,
        ...
        if self.is_turbo:
            ...
            self.scheduler.set_timesteps(len(timesteps), device='cuda')
